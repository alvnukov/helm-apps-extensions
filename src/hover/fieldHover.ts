...
const GROUP_COMPONENT_HINTS: Record<string, Record<string, { en: string; ru: string }>> = {
  "apps-stateless": {
    ...
    replicas: { en: "Desired replica count for Deployment.", ru: "Желаемое количество реплик Deployment." },
    strategy: { en: "Deployment rollout strategy and surge/unavailable limits.", ru: "Стратегия rollout Deployment и лимиты surge/unavailable." },
    selector: { en: "Pod selector used by workload and related resources.", ru: "Selector pod-ов, используемый workload-ом и связанными ресурсами." },
  ...
  "apps-stateful": {
    ...
    replicas: { en: "Desired StatefulSet replicas count.", ru: "Желаемое количество реплик StatefulSet." },
    serviceName: { en: "Headless service name used by StatefulSet network identity.", ru: "Имя headless service для сетевой идентичности StatefulSet." },
    podManagementPolicy: { en: "StatefulSet pod creation/deletion ordering behavior.", ru: "Поведение порядка создания/удаления pod в StatefulSet." },
  ...
  "apps-jobs": {
    ...
    completionMode: { en: "How Job tracks completion state (`NonIndexed`/`Indexed`).", ru: "Как Job отслеживает завершение (`NonIndexed`/`Indexed`)." },
  ...
  "apps-cronjobs": {
    ...
    suspend: { en: "Temporarily pauses scheduled launches.", ru: "Временно приостанавливает плановые запуски." },
    completionMode: { en: "Completion tracking mode for spawned Jobs.", ru: "Режим отслеживания завершения для создаваемых Job." },
  ...
  "apps-services": {
    ...
    clusterIP: { en: "Fixed virtual IP for Service (or `None` in headless mode).", ru: "Фиксированный виртуальный IP Service (или `None` для headless-режима)." },
    sessionAffinity: { en: "Client-IP affinity behavior for Service endpoints.", ru: "Поведение affinity по клиентскому IP для endpoint-ов Service." },
    externalTrafficPolicy: { en: "Preserve source IP / traffic routing policy for NodePort/LoadBalancer.", ru: "Политика source IP и маршрутизации трафика для NodePort/LoadBalancer." },
    internalTrafficPolicy: { en: "In-cluster routing preference (`Cluster` or `Local`).", ru: "Политика внутрикластерной маршрутизации (`Cluster` или `Local`)." },
    loadBalancerIP: { en: "Static IP request for LoadBalancer service.", ru: "Запрос статического IP для LoadBalancer сервиса." },
    extraSpec: { en: "Raw Service spec patch merged after generated fields.", ru: "Raw-патч Service spec, мержимый после сгенерированных полей." },
  },
  "apps-service-accounts": {
    ...
    imagePullSecrets: { en: "Image pull secret references attached to this ServiceAccount.", ru: "Ссылки на image pull secrets, привязанные к этому ServiceAccount." },
    secrets: { en: "Additional secrets mounted by default for pods using this ServiceAccount.", ru: "Дополнительные secrets, автоматически монтируемые в pod с этим ServiceAccount." },
    apiVersion: { en: "ServiceAccount API version override (defaults to v1).", ru: "Переопределение API версии ServiceAccount (по умолчанию v1)." },
    extraFields: { en: "Raw top-level ServiceAccount fields for advanced native options.", ru: "Raw top-level поля ServiceAccount для расширенных нативных опций." },
  },
  "apps-ingresses": {
...
const RULES: DocRule[] = [
  ...
  {
    pattern: ["apps-services", "*", "ports"],
    doc: {
      title: "Standalone Service Ports",
      titleRu: "Порты standalone Service",
      summary: "Ports exposed by standalone service resource.",
      summaryRu: "Порты, публикуемые standalone service-ресурсом.",
      type: "YAML block string | list | env-map",
      docsLink: "docs/reference-values.md#param-service",
      k8sDocsLink: "https://kubernetes.io/docs/concepts/services-networking/service/",
      example: "ports: |-\n  - name: http\n    port: 80\n    targetPort: 8080\n",
  ...
  {
    pattern: ["apps-services", "*", "selector"],
    doc: {
      title: "Standalone Service Selector",
      titleRu: "Selector standalone Service",
      summary: "Label selector binding service to target pods.",
      summaryRu: "Label selector, связывающий service с целевыми pod.",
      type: "YAML block string | map | env-map",
      docsLink: "docs/reference-values.md#param-service",
      k8sDocsLink: "https://kubernetes.io/docs/concepts/services-networking/service/",
  ...
  {
    pattern: ["apps-services", "*", "headless"],
    doc: {
      title: "Standalone Headless Service",
      titleRu: "Headless режим standalone Service",
      summary: "Switches service to headless mode (`clusterIP: None`).",
      summaryRu: "Переводит service в headless-режим (`clusterIP: None`).",
      type: "bool | env-map",
      docsLink: "docs/reference-values.md#param-service",
  ...
  {
    pattern: ["apps-services", "*", "clusterIP"],
    doc: {
      title: "Service ClusterIP",
      titleRu: "ClusterIP Service",
      summary: "Assigns explicit ClusterIP (or `None` for headless behavior).",
      summaryRu: "Задает явный ClusterIP (или `None` для headless-поведения).",
      type: "string | env-map",
      docsLink: "docs/reference-values.md#param-service",
      k8sDocsLink: "https://kubernetes.io/docs/concepts/services-networking/service/#publishing-services-service-types",
      example: "clusterIP: 10.96.10.25\n",
    },
  },
  {
    pattern: ["apps-services", "*", "sessionAffinity"],
    doc: {
      title: "Service Session Affinity",
      titleRu: "Session affinity Service",
      summary: "Client session affinity policy for service endpoints.",
      summaryRu: "Политика сессионной affinity клиента к endpoint-ам service.",
      type: "string | env-map",
      docsLink: "docs/reference-values.md#param-service",
      k8sDocsLink: "https://kubernetes.io/docs/reference/networking/virtual-ips/#session-affinity",
      example: "sessionAffinity: ClientIP\n",
    },
  },
  {
    pattern: ["apps-services", "*", "externalTrafficPolicy"],
    doc: {
      title: "External Traffic Policy",
      titleRu: "Политика внешнего трафика",
      summary: "Controls external traffic routing behavior for NodePort/LoadBalancer.",
      summaryRu: "Управляет маршрутизацией внешнего трафика для NodePort/LoadBalancer.",
      type: "Cluster | Local | env-map",
      docsLink: "docs/reference-values.md#param-service",
      k8sDocsLink: "https://kubernetes.io/docs/reference/networking/virtual-ips/#traffic-policies",
      example: "externalTrafficPolicy: Local\n",
    },
  },
  {
    pattern: ["apps-services", "*", "internalTrafficPolicy"],
    doc: {
      title: "Internal Traffic Policy",
      titleRu: "Политика внутреннего трафика",
      summary: "Controls in-cluster traffic routing preference (`Cluster`/`Local`).",
      summaryRu: "Управляет внутрикластерной маршрутизацией (`Cluster`/`Local`).",
      type: "Cluster | Local | env-map",
      docsLink: "docs/reference-values.md#param-service",
      k8sDocsLink: "https://kubernetes.io/docs/concepts/services-networking/service-traffic-policy/",
      example: "internalTrafficPolicy: Cluster\n",
    },
  },
  {
    pattern: ["apps-services", "*", "loadBalancerIP"],
    doc: {
      title: "LoadBalancer Static IP",
      titleRu: "Статический IP для LoadBalancer",
      summary: "Requests a specific IP for LoadBalancer service (provider-dependent).",
      summaryRu: "Запрашивает конкретный IP для LoadBalancer service (зависит от провайдера).",
      type: "string | env-map",
      docsLink: "docs/reference-values.md#param-service",
      k8sDocsLink: "https://kubernetes.io/docs/concepts/services-networking/service/#loadbalancer",
      example: "loadBalancerIP: 203.0.113.42\n",
    },
  },
  {
    pattern: ["apps-services", "*", "extraSpec"],
    doc: {
      title: "Service Extra Spec Patch",
      titleRu: "Дополнительный патч spec для Service",
      summary: "Raw fragment merged into generated Service spec for advanced options.",
      summaryRu: "Raw-фрагмент, мержимый в сгенерированный Service spec для расширенных опций.",
      type: "YAML block string | map | env-map",
      docsLink: "docs/reference-values.md#param-service",
  ...
  },
  {
    ...
    doc: {
      title: "Legacy Ingress servicePort (unsupported)",
      titleRu: "Legacy ingress servicePort (не поддерживается)",
      summary: "Legacy key from older contracts; current built-in ingress renderer expects full `paths` backend block.",
      summaryRu: "Legacy-ключ старых контрактов; текущий встроенный ingress-рендер ожидает полный backend-блок внутри `paths`.",
  ...
  {
    pattern: ["apps-ingresses", "*", "host"],
    doc: {
      title: "Ingress Host",
      titleRu: "Ingress host",
      summary: "Primary host/domain for ingress rule.",
      summaryRu: "Основной host/domain для ingress-правила.",
      type: "string | env-map",
      docsLink: "docs/reference-values.md#param-ingress",
      k8sDocsLink: "https://kubernetes.io/docs/concepts/services-networking/ingress/",
      example: "host: app.example.local\n",
    },
  },
  {
    pattern: ["apps-ingresses", "*", "ingressClassName"],
    doc: {
      title: "Ingress Class Name",
      titleRu: "Имя IngressClass",
      summary: "Ingress class for selecting the ingress controller.",
      summaryRu: "Ingress class для выбора ingress-контроллера.",
      type: "string | env-map",
      docsLink: "docs/reference-values.md#param-ingress",
  ...
  {
    pattern: ["apps-ingresses", "*", "class"],
    doc: {
      title: "Ingress Class (legacy annotation)",
      titleRu: "Ingress class (legacy annotation)",
      summary: "Legacy ingress class hint used via annotation compatibility path.",
      summaryRu: "Legacy-подсказка класса ingress через путь совместимости аннотаций.",
      type: "string | env-map",
      docsLink: "docs/reference-values.md#param-ingress",
      example: "class: nginx\n",
    },
  ...
  {
    pattern: ["apps-ingresses", "*", "tls", "secret_name"],
    doc: {
      title: "Ingress TLS Secret Name",
      titleRu: "Имя TLS-секрета для Ingress",
      summary: "TLS secret name used by ingress; when omitted, certificate helper can derive default.",
      summaryRu: "Имя TLS-секрета для ingress; если не задано, helper сертификата может взять дефолт.",
      type: "string | env-map",
      docsLink: "docs/reference-values.md#param-ingress",
      k8sDocsLink: "https://kubernetes.io/docs/concepts/services-networking/ingress/#tls",
  ...
  {
    pattern: ["apps-ingresses", "*", "dexAuth", "clusterDomain"],
    doc: {
      title: "Dex Auth Cluster Domain",
      titleRu: "Cluster domain для Dex Auth",
      summary: "Cluster DNS domain used to build in-cluster auth-url for dex-authenticator.",
      summaryRu: "Cluster DNS domain для сборки in-cluster auth-url к dex-authenticator.",
      type: "string | env-map",
      docsLink: "docs/reference-values.md#param-ingress",
  ...
  {
    pattern: ["apps-ingresses", "*", "extraSpec"],
    doc: {
      title: "Ingress Extra Spec Patch",
      titleRu: "Дополнительный патч spec для Ingress",
      summary: "Raw fragment merged into generated ingress spec after base fields.",
      summaryRu: "Raw-фрагмент, мержимый в сгенерированный ingress spec после базовых полей.",
      type: "YAML block string | map | env-map",
      docsLink: "docs/reference-values.md#param-ingress",
      example: "extraSpec: |-\n  defaultBackend:\n    service:\n      name: fallback\n      port:\n        number: 80\n",
    },
  ...
  {
    pattern: ["apps-service-accounts", "*", "imagePullSecrets"],
    doc: {
      title: "ServiceAccount ImagePullSecrets",
      titleRu: "ImagePullSecrets в ServiceAccount",
      summary: "Optional image pull secrets attached to ServiceAccount.",
      summaryRu: "Опциональные image pull secrets, привязанные к ServiceAccount.",
      type: "YAML block string with secret refs | list",
      docsLink: "docs/reference-values.md#param-apps-sections",
  ...
  {
    pattern: ["apps-service-accounts", "*", "secrets"],
    doc: {
      title: "ServiceAccount Default Secrets",
      titleRu: "Список secrets в ServiceAccount",
      summary: "Optional list of secrets referenced by ServiceAccount.",
      summaryRu: "Опциональный список secrets, на которые ссылается ServiceAccount.",
      type: "YAML block string with secret refs | list",
      docsLink: "docs/reference-values.md#param-apps-sections",
      k8sDocsLink: "https://kubernetes.io/docs/concepts/security/service-accounts/",
      example: "secrets: |-\n  - name: app-bootstrap-token\n",
    },
  },
  {
    pattern: ["apps-service-accounts", "*", "apiVersion"],
    doc: {
      title: "ServiceAccount ApiVersion",
      titleRu: "ApiVersion ServiceAccount",
      summary: "API version override for generated ServiceAccount object.",
      summaryRu: "Переопределение API версии для генерируемого ServiceAccount.",
      type: "string | env-map",
      docsLink: "docs/reference-values.md#param-apps-sections",
      example: "apiVersion: v1\n",
    },
  ...
  {
    pattern: ["apps-service-accounts", "*", "extraFields"],
    doc: {
      title: "ServiceAccount Extra Fields",
      titleRu: "Дополнительные поля ServiceAccount",
      summary: "Raw top-level fields merged into generated ServiceAccount.",
      summaryRu: "Raw top-level поля, которые мержатся в сгенерированный ServiceAccount.",
  ...
  },
  {
    ...
    doc: {
      title: "Legacy clusterRole Key",
      titleRu: "Legacy-ключ clusterRole",
      summary: "`apps-service-accounts` renderer does not consume root `clusterRole`; use `clusterRoles` map entries instead.",
      summaryRu: "Рендерер `apps-service-accounts` не обрабатывает root `clusterRole`; используйте элементы в `clusterRoles`.",
      type: "legacy/unsupported in this group",
      docsLink: "docs/reference-values.md#param-apps-sections",
      k8sDocsLink: "https://kubernetes.io/docs/reference/access-authn-authz/rbac/",
      notes: [
        "Use `clusterRoles.<name>.rules` to generate ClusterRole + ClusterRoleBinding pair.",
      ],
      notesRu: [
        "Используйте `clusterRoles.<name>.rules`, чтобы сгенерировать пару ClusterRole + ClusterRoleBinding.",
      ],
      example: "clusterRoles:\n  viewer:\n    rules: |-\n      - apiGroups: [\"*\"]\n        resources: [\"*\"]\n        verbs: [\"get\", \"list\", \"watch\"]\n",
    },
  },
  {
    ...
    doc: {
      title: "Role Entry",
      titleRu: "Элемент Role",
      summary: "Named namespaced Role + RoleBinding entry for this ServiceAccount.",
      summaryRu: "Именованный элемент namespaced Role + RoleBinding для этого ServiceAccount.",
      type: "map",
      docsLink: "docs/reference-values.md#param-apps-sections",
      k8sDocsLink: "https://kubernetes.io/docs/reference/access-authn-authz/rbac/#role-and-clusterrole",
      notes: [
        "Default Role name is `<serviceAccountName>-<roleKey>` unless `name` is set.",
        "Default binding subject is current ServiceAccount unless `binding.subjects` is set.",
      ],
      notesRu: [
        "Имя Role по умолчанию: `<serviceAccountName>-<roleKey>`, если не задан `name`.",
        "Subject в binding по умолчанию — текущий ServiceAccount, если не задан `binding.subjects`.",
      ],
      example: "roles:\n  pod-reader:\n    name: app-pod-reader\n    rules: |-\n      - apiGroups: [\"\"]\n        resources: [\"pods\"]\n        verbs: [\"get\", \"list\"]\n",
    },
  },
  {
    pattern: ["apps-service-accounts", "*", "clusterRoles", "*"],
    doc: {
      title: "ClusterRole Entry",
      titleRu: "Элемент ClusterRole",
      summary: "Named cluster-scoped ClusterRole + ClusterRoleBinding entry for this ServiceAccount.",
      summaryRu: "Именованный cluster-scoped элемент ClusterRole + ClusterRoleBinding для этого ServiceAccount.",
      type: "map",
      docsLink: "docs/reference-values.md#param-apps-sections",
      k8sDocsLink: "https://kubernetes.io/docs/reference/access-authn-authz/rbac/#role-and-clusterrole",
      notes: [
        "Use this for permissions that must work cluster-wide.",
      ],
      notesRu: [
        "Используйте для прав, которые должны работать на уровне всего кластера.",
      ],
      example: "clusterRoles:\n  viewer:\n    rules: |-\n      - apiGroups: [\"*\"]\n        resources: [\"*\"]\n        verbs: [\"get\", \"list\", \"watch\"]\n",
    },
  },
  {
    pattern: ["apps-service-accounts", "*", "*", "*", "name"],
    doc: {
      title: "RBAC Object Name Override",
      titleRu: "Переопределение имени RBAC-объекта",
      summary: "Overrides generated Role/ClusterRole name for this RBAC entry.",
      summaryRu: "Переопределяет сгенерированное имя Role/ClusterRole для текущего RBAC-элемента.",
      type: "string | env-map",
      docsLink: "docs/reference-values.md#param-apps-sections",
      example: "roles:\n  pod-reader:\n    name: app-pod-reader\n",
    },
  ...
  {
    pattern: ["apps-service-accounts", "*", "*", "*", "binding"],
    doc: {
      title: "RBAC Binding Block",
      titleRu: "Блок RBAC binding",
      summary: "Binding options for generated RoleBinding/ClusterRoleBinding.",
      summaryRu: "Опции binding для генерируемого RoleBinding/ClusterRoleBinding.",
      type: "map",
      docsLink: "docs/reference-values.md#param-apps-sections",
      k8sDocsLink: "https://kubernetes.io/docs/reference/access-authn-authz/rbac/#rolebinding-and-clusterrolebinding",
      example: "binding:\n  name: app-pod-reader-binding\n",
    },
  },
  {
    pattern: ["apps-service-accounts", "*", "*", "*", "binding", "name"],
    doc: {
      title: "Binding Name Override",
      titleRu: "Переопределение имени binding",
      summary: "Overrides generated RoleBinding/ClusterRoleBinding object name.",
      summaryRu: "Переопределяет имя генерируемого RoleBinding/ClusterRoleBinding.",
      type: "string | env-map",
      docsLink: "docs/reference-values.md#param-apps-sections",
  ...
  {
    pattern: ["apps-service-accounts", "*", "*", "*", "binding", "extraFields"],
    doc: {
      title: "Binding Extra Fields",
      titleRu: "Дополнительные поля binding",
      summary: "Raw top-level fields merged into generated binding object.",
      summaryRu: "Raw top-level поля, мержимые в генерируемый binding-объект.",
      type: "YAML block string | map",
      docsLink: "docs/reference-values.md#param-apps-sections",
      example: "binding:\n  extraFields:\n    metadata:\n      annotations:\n        owner: platform\n",
    },
  },
  {
    pattern: ["apps-service-accounts", "*", "*", "*", "extraFields"],
    doc: {
      title: "RBAC Role Extra Fields",
      titleRu: "Дополнительные поля RBAC-роли",
      summary: "Raw top-level fields merged into generated Role/ClusterRole object.",
      summaryRu: "Raw top-level поля, мержимые в генерируемый Role/ClusterRole объект.",
      type: "YAML block string | map",
      docsLink: "docs/reference-values.md#param-apps-sections",
  ...
  {
    pattern: ["apps-service-accounts", "*", "*", "*", "rules", "*"],
    doc: {
      title: "RBAC Rule Entry",
      titleRu: "Элемент RBAC-правила",
      summary: "Named RBAC rule item used inside `rules` map.",
      summaryRu: "Именованный элемент RBAC-правила внутри карты `rules`.",
      type: "map",
      docsLink: "docs/reference-values.md#param-apps-sections",
      k8sDocsLink: "https://kubernetes.io/docs/reference/access-authn-authz/rbac/#referring-to-resources",
      notes: [
        "Set `verbs` and at least one of `resources` or `nonResourceURLs`.",
      ],
      notesRu: [
        "Задайте `verbs` и хотя бы одно из полей: `resources` или `nonResourceURLs`.",
      ],
      example: "rules:\n  pod-reader:\n    verbs: |-\n      - get\n      - list\n    resources: |-\n      - pods\n",
    },
  },
  {
    pattern: ["apps-service-accounts", "*", "*", "*", "rules", "*", "enabled"],
    doc: {
      title: "RBAC Rule Enable Flag",
      titleRu: "Флаг включения RBAC-правила",
      summary: "Enables/disables a single named RBAC rule entry.",
      summaryRu: "Включает/выключает отдельный именованный элемент RBAC-правила.",
      type: "bool | env-map",
      docsLink: "docs/reference-values.md#param-apps-sections",
      example: "rules:\n  pod-reader:\n    enabled: false\n",
    },
  ...
  {
    pattern: ["apps-service-accounts", "*", "*", "*", "rules", "*", "apiGroups"],
    doc: {
      title: "RBAC Rule ApiGroups",
      titleRu: "ApiGroups в RBAC-правиле",
      summary: "API groups list for this rule (for core group use empty string).",
      summaryRu: "Список API-групп в этом правиле (для core-группы используйте пустую строку).",
      type: "YAML block string with string list | list",
      docsLink: "docs/reference-values.md#param-apps-sections",
      example: "apiGroups: |-\n  - \"\"\n  - apps\n",
    },
  },
  {
    pattern: ["apps-service-accounts", "*", "*", "*", "rules", "*", "resources"],
    doc: {
      title: "RBAC Rule Resources",
      titleRu: "Resources в RBAC-правиле",
      summary: "Resource names (plural) that this rule grants access to.",
      summaryRu: "Имена ресурсов (во множественном числе), к которым правило дает доступ.",
      type: "YAML block string with string list | list",
      docsLink: "docs/reference-values.md#param-apps-sections",
      example: "resources: |-\n  - pods\n  - deployments\n",
    },
  ...
  {
    pattern: ["apps-service-accounts", "*", "*", "*", "rules", "*", "verbs"],
    doc: {
      title: "RBAC Rule Verbs",
      titleRu: "Verbs в RBAC-правиле",
      summary: "Allowed actions for this rule (`get`, `list`, `watch`, ...).",
      summaryRu: "Разрешенные действия в правиле (`get`, `list`, `watch` и т.д.).",
      type: "YAML block string with string list | list",
      docsLink: "docs/reference-values.md#param-apps-sections",
      example: "verbs: |-\n  - get\n  - list\n  - watch\n",
    },
  ...
  {
    pattern: ["apps-service-accounts", "*", "*", "*", "rules", "*", "resourceNames"],
    doc: {
      title: "RBAC Rule ResourceNames",
      titleRu: "ResourceNames в RBAC-правиле",
      summary: "Optional narrow list of concrete resource names for this rule.",
      summaryRu: "Опциональный узкий список конкретных имен ресурсов для этого правила.",
      type: "YAML block string with string list | list",
      docsLink: "docs/reference-values.md#param-apps-sections",
      example: "resourceNames: |-\n  - app-config\n",
    },
  },
  {
    pattern: ["apps-service-accounts", "*", "*", "*", "rules", "*", "nonResourceURLs"],
    doc: {
      title: "RBAC Rule NonResourceURLs",
      titleRu: "NonResourceURLs в RBAC-правиле",
      summary: "Non-resource URL paths for API server endpoints.",
      summaryRu: "Non-resource URL пути для endpoint-ов API сервера.",
      type: "YAML block string with string list | list",
      docsLink: "docs/reference-values.md#param-apps-sections",
      example: "nonResourceURLs: |-\n  - /metrics\n  - /healthz\n",
    },
  },
  {
    pattern: ["apps-service-accounts", "*", "*", "*", "rules", "*", "extraFields"],
    doc: {
      title: "RBAC Rule Extra Fields",
      titleRu: "Дополнительные поля RBAC-правила",
      summary: "Raw fields merged into rendered RBAC rule item.",
      summaryRu: "Raw-поля, которые мержатся в отрендеренный элемент RBAC-правила.",
      type: "YAML block string | map",
      docsLink: "docs/reference-values.md#param-apps-sections",
...
const LAST_KEY_RULES: Record<string, FieldDoc> = {
  ...
  reloader: {
    title: "Reloader Annotation Toggle",
    titleRu: "Флаг reloader-аннотации",
    summary: "Enables Stakater Reloader style annotation on workload pod template.",
    summaryRu: "Включает аннотацию в стиле Stakater Reloader на pod template workload-а.",
    type: "bool | env-map",
    notes: [
      "When enabled, workload can restart automatically on related ConfigMap/Secret changes.",
    ],
    notesRu: [
      "При включении workload может автоматически перезапускаться при изменениях связанных ConfigMap/Secret.",
    ],
    example: "reloader: true\n",
  },
  replicas: {
    title: "Replicas Count",
    titleRu: "Количество реплик",
    summary: "Desired number of workload replicas (Deployment/StatefulSet and similar controllers).",
    summaryRu: "Желаемое количество реплик workload-а (Deployment/StatefulSet и аналогичные контроллеры).",
    type: "number | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/workloads/controllers/deployment/",
    example: "replicas: 3\n",
  },
  strategy: {
    title: "Update Strategy",
    titleRu: "Стратегия обновления",
    summary: "Controller update strategy (for example Deployment rolling update settings).",
    summaryRu: "Стратегия обновления контроллера (например параметры rolling update у Deployment).",
    type: "YAML block string | map | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/workloads/controllers/deployment/#strategy",
    example: "strategy: |-\n  type: RollingUpdate\n  rollingUpdate:\n    maxSurge: 25%\n    maxUnavailable: 25%\n",
  },
  minReadySeconds: {
    title: "Min Ready Seconds",
    titleRu: "Минимальное время готовности",
    summary: "Minimum seconds a pod should be ready before being considered available.",
    summaryRu: "Минимальное число секунд, которое pod должен быть готов перед учетом как available.",
    type: "number | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/workloads/controllers/deployment/#min-ready-seconds",
    example: "minReadySeconds: 5\n",
  },
  progressDeadlineSeconds: {
    title: "Progress Deadline Seconds",
    titleRu: "Дедлайн прогресса rollout (сек)",
    summary: "Timeout for rollout progress before controller marks deployment as failed.",
    summaryRu: "Таймаут прогресса rollout, после которого контроллер помечает деплой как failed.",
    type: "number | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/workloads/controllers/deployment/#progress-deadline-seconds",
    example: "progressDeadlineSeconds: 600\n",
  },
  revisionHistoryLimit: {
    title: "Revision History Limit",
    titleRu: "Лимит истории ревизий",
    summary: "How many old revisions controller keeps for rollback/history.",
    summaryRu: "Сколько старых ревизий контроллер хранит для rollback/истории.",
    type: "number | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/workloads/controllers/deployment/#revision-history-limit",
    example: "revisionHistoryLimit: 3\n",
  },
  serviceName: {
    title: "StatefulSet Service Name",
    titleRu: "ServiceName для StatefulSet",
    summary: "Headless service name used by StatefulSet for stable network identities.",
    summaryRu: "Имя headless service, которое StatefulSet использует для стабильных сетевых идентичностей.",
    type: "string | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/",
    example: "serviceName: app-headless\n",
  },
  podManagementPolicy: {
    title: "Pod Management Policy",
    titleRu: "Политика управления pod-ами",
    summary: "StatefulSet pod creation/termination ordering policy (`OrderedReady` or `Parallel`).",
    summaryRu: "Политика порядка создания/удаления pod в StatefulSet (`OrderedReady` или `Parallel`).",
    type: "OrderedReady | Parallel | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/#pod-management-policies",
    example: "podManagementPolicy: OrderedReady\n",
  },
  updateStrategy: {
    title: "StatefulSet Update Strategy",
    titleRu: "Стратегия обновления StatefulSet",
    summary: "Defines how StatefulSet updates pods (`RollingUpdate` / `OnDelete`).",
    summaryRu: "Определяет, как StatefulSet обновляет pod (`RollingUpdate` / `OnDelete`).",
    type: "YAML block string | map | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/#update-strategies",
    example: "updateStrategy: |-\n  type: RollingUpdate\n",
  },
  volumeClaimTemplates: {
    title: "Volume Claim Templates",
    titleRu: "Шаблоны VolumeClaim",
    summary: "StatefulSet PVC templates used to provision per-pod persistent volumes.",
    summaryRu: "Шаблоны PVC для StatefulSet, которые создают per-pod persistent volume.",
    type: "YAML block string | list | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/#stable-storage",
    example: "volumeClaimTemplates: |-\n  - metadata:\n      name: data\n    spec:\n      accessModes: [\"ReadWriteOnce\"]\n      resources:\n        requests:\n          storage: 10Gi\n",
  },
  persistentVolumeClaimRetentionPolicy: {
    title: "PVC Retention Policy",
    titleRu: "Политика удержания PVC",
    summary: "Controls whether StatefulSet PVCs are retained or deleted on scale/delete events.",
    summaryRu: "Управляет тем, сохраняются или удаляются PVC StatefulSet при scale/delete событиях.",
    type: "YAML block string | map | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/#persistentvolumeclaim-retention",
    example: "persistentVolumeClaimRetentionPolicy: |-\n  whenDeleted: Retain\n  whenScaled: Retain\n",
  },
  completionMode: {
    title: "Job Completion Mode",
    titleRu: "Режим завершения Job",
    summary: "Controls how Job tracks completion (`NonIndexed` or `Indexed`).",
    summaryRu: "Определяет, как Job отслеживает завершение (`NonIndexed` или `Indexed`).",
    type: "NonIndexed | Indexed | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/workloads/controllers/job/",
    example: "completionMode: NonIndexed\n",
  },
  completions: {
    title: "Job Completions",
    titleRu: "Количество завершений Job",
    summary: "Number of successfully finished pods required for Job completion.",
    summaryRu: "Количество успешно завершенных pod, необходимых для завершения Job.",
    type: "number | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/workloads/controllers/job/",
  ...
  parallelism: {
    ...
    titleRu: "Параллелизм Job",
    summary: "Maximum number of pods running in parallel for the Job.",
    summaryRu: "Максимальное количество pod, выполняющихся параллельно для Job.",
    type: "number | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/workloads/controllers/job/",
  ...
  manualSelector: {
    ...
    titleRu: "Ручной selector",
    summary: "Enables manual label selector management for Job pod selection.",
    summaryRu: "Включает ручное управление label selector для выбора pod Job.",
    type: "boolean | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/workloads/controllers/job/",
    example: "manualSelector: false\n",
  },
  suspend: {
    title: "Suspend Execution",
    titleRu: "Приостановить выполнение",
    summary: "Suspends Job/CronJob execution without deleting existing history.",
    summaryRu: "Приостанавливает выполнение Job/CronJob без удаления существующей истории.",
    type: "boolean | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/workloads/controllers/cron-jobs/",
    example: "suspend: false\n",
  },
  ttlSecondsAfterFinished: {
    title: "TTL Seconds After Finished",
    titleRu: "TTL после завершения (сек)",
    summary: "Time-to-live for finished Job objects before automatic cleanup.",
    summaryRu: "Время жизни завершенных Job-объектов до автоматической очистки.",
    type: "number | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/workloads/controllers/ttlafterfinished/",
    example: "ttlSecondsAfterFinished: 86400\n",
  },
  jobTemplateExtraSpec: {
    title: "Job Template Extra Spec",
    titleRu: "Дополнительный spec для Job template",
    summary: "Raw patch merged into generated `jobTemplate.spec`.",
    summaryRu: "Raw-патч, мержимый в сгенерированный `jobTemplate.spec`.",
    type: "YAML block string | map | env-map",
    example: "jobTemplateExtraSpec: |-\n  template:\n    spec:\n      priorityClassName: batch-high\n",
  },
  podSpecExtra: {
    title: "Pod Spec Extra Patch",
    titleRu: "Дополнительный патч pod spec",
    summary: "Raw fragment merged into generated pod template spec.",
    summaryRu: "Raw-фрагмент, мержимый в сгенерированный pod template spec.",
    type: "YAML block string | map | env-map",
    example: "podSpecExtra: |-\n  runtimeClassName: gvisor\n",
  },
  schedule: {
    title: "Cron Schedule",
  ...
  terminationGracePeriodSeconds: {
    ...
    titleRu: "Время graceful shutdown",
    summary: "How long Kubernetes waits before force-killing pod containers on termination.",
    summaryRu: "Сколько Kubernetes ждет перед принудительным завершением контейнеров pod при остановке.",
    type: "number | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle/#pod-termination",
    example: "terminationGracePeriodSeconds: 60\n",
  },
  serviceAccountName: {
    title: "ServiceAccount Name Override",
    titleRu: "Переопределение имени ServiceAccount",
    summary: "Direct pod spec serviceAccountName override for workload pods.",
    summaryRu: "Прямое переопределение serviceAccountName в pod spec workload-а.",
    type: "string | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/tasks/configure-pod-container/configure-service-account/",
    example: "serviceAccountName: app-runtime\n",
  },
  runtimeClassName: {
    title: "RuntimeClass Name",
    titleRu: "Имя RuntimeClass",
    summary: "Selects container runtime handler for pod sandbox.",
    summaryRu: "Выбирает runtime handler контейнеров для pod sandbox.",
    type: "string | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/containers/runtime-class/",
    example: "runtimeClassName: gvisor\n",
  },
  schedulerName: {
    title: "Scheduler Name",
    titleRu: "Имя scheduler-а",
    summary: "Selects custom scheduler for pod placement.",
    summaryRu: "Выбирает кастомный scheduler для размещения pod.",
    type: "string | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/tasks/extend-kubernetes/configure-multiple-schedulers/",
    example: "schedulerName: custom-scheduler\n",
  },
  dnsPolicy: {
    title: "Pod DNS Policy",
    titleRu: "DNS policy pod-а",
    summary: "DNS policy for pod (`ClusterFirst`, `Default`, `None`, ...).",
    summaryRu: "DNS policy для pod (`ClusterFirst`, `Default`, `None` и т.д.).",
    type: "string | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/services-networking/dns-pod-service/#pods-dns-policy",
    example: "dnsPolicy: ClusterFirst\n",
  },
  dnsConfig: {
    title: "Pod DNS Config",
    titleRu: "DNS config pod-а",
    summary: "Additional DNS resolver configuration merged into pod spec.",
    summaryRu: "Дополнительная DNS-конфигурация резолвера, мержимая в pod spec.",
    type: "YAML block string | map | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/services-networking/dns-pod-service/#pod-dns-config",
    example: "dnsConfig: |-\n  options:\n    - name: ndots\n      value: \"2\"\n",
  },
  hostAliases: {
    title: "Host Aliases",
    titleRu: "Host aliases",
    summary: "Static `/etc/hosts` entries injected into pod containers.",
    summaryRu: "Статические записи `/etc/hosts`, инжектируемые в контейнеры pod.",
    type: "YAML block string | list | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/tasks/network/customize-hosts-file-for-pods/",
    example: "hostAliases: |-\n  - ip: \"10.1.2.3\"\n    hostnames:\n      - legacy-db.local\n",
  },
  hostname: {
    title: "Pod Hostname",
    titleRu: "Имя хоста pod-а",
    summary: "Overrides pod hostname inside cluster DNS scope.",
    summaryRu: "Переопределяет hostname pod внутри DNS-области кластера.",
    type: "string | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/workloads/pods/pod-hostname/",
    example: "hostname: api-0\n",
  },
  nodeName: {
    title: "Pinned Node Name",
    titleRu: "Фиксированное имя ноды",
    summary: "Hard-pins pod to exact node name (bypasses scheduler decisions).",
    summaryRu: "Жестко закрепляет pod за конкретной нодой (обходит обычное планирование scheduler-ом).",
    type: "string | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#nodename",
    example: "nodeName: worker-1\n",
  },
  subdomain: {
    title: "Pod Subdomain",
    titleRu: "Subdomain pod-а",
    summary: "Subdomain used with hostname to build stable pod DNS records.",
    summaryRu: "Subdomain, используемый с hostname для стабильных DNS-записей pod.",
    type: "string | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/workloads/pods/pod-hostname/#hostname-with-pod-subdomain",
    example: "subdomain: app-headless\n",
  },
  preemptionPolicy: {
    title: "Preemption Policy",
    titleRu: "Политика preemption",
    summary: "Controls whether pod can preempt lower-priority pods (`PreemptLowerPriority`/`Never`).",
    summaryRu: "Управляет тем, может ли pod вытеснять pod с меньшим приоритетом (`PreemptLowerPriority`/`Never`).",
    type: "string | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/scheduling-eviction/pod-priority-preemption/",
    example: "preemptionPolicy: PreemptLowerPriority\n",
  },
  priority: {
    title: "Pod Priority Value",
    titleRu: "Значение приоритета pod-а",
    summary: "Numeric pod priority value (usually managed via `priorityClassName`).",
    summaryRu: "Числовое значение приоритета pod (обычно задается через `priorityClassName`).",
    type: "number | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/scheduling-eviction/pod-priority-preemption/",
    example: "priority: 1000000\n",
  },
  automountServiceAccountToken: {
    title: "Pod Automount SA Token",
    titleRu: "Автомонтирование SA-токена в pod",
    summary: "Pod-level override for automatic ServiceAccount token mount.",
    summaryRu: "Pod-level переопределение автоматического монтирования ServiceAccount token.",
    type: "bool | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/tasks/configure-pod-container/configure-service-account/#opt-out-of-api-credential-automounting",
    example: "automountServiceAccountToken: false\n",
  },
  enableServiceLinks: {
    ...
    titleRu: "Включить service links",
    summary: "Controls automatic service environment variables injection into pod containers.",
    summaryRu: "Управляет автоматическим инжектом service env-переменных в контейнеры pod.",
    type: "bool | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/services-networking/connect-applications-service/",
    example: "enableServiceLinks: false\n",
  },
  hostIPC: {
    title: "Host IPC Namespace",
    titleRu: "Host IPC namespace",
    summary: "Shares host IPC namespace with pod (advanced/security-sensitive).",
    summaryRu: "Делит host IPC namespace с pod (продвинутый/чувствительный параметр безопасности).",
    type: "bool | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.30/#podspec-v1-core",
    example: "hostIPC: false\n",
  },
  hostNetwork: {
    title: "Host Network Mode",
    titleRu: "Режим host network",
    summary: "Runs pod in host network namespace (ports/conflicts must be managed explicitly).",
    summaryRu: "Запускает pod в host network namespace (порты/конфликты нужно контролировать явно).",
    type: "bool | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.30/#podspec-v1-core",
    example: "hostNetwork: false\n",
  },
  hostPID: {
    title: "Host PID Namespace",
    titleRu: "Host PID namespace",
    summary: "Shares host PID namespace with pod (advanced/debug/security-sensitive).",
    summaryRu: "Делит host PID namespace с pod (продвинутое/debug/чувствительное поле).",
    type: "bool | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.30/#podspec-v1-core",
    example: "hostPID: false\n",
  },
  setHostnameAsFQDN: {
    title: "Set Hostname As FQDN",
    titleRu: "Использовать FQDN как hostname",
    summary: "Makes kernel hostname equal to pod FQDN instead of short host.",
    summaryRu: "Делает kernel hostname равным полному FQDN pod-а вместо короткого имени.",
    type: "bool | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/workloads/pods/pod-hostname/#hostname-with-pod-fqdn",
    example: "setHostnameAsFQDN: true\n",
  },
  shareProcessNamespace: {
    title: "Share Process Namespace",
    titleRu: "Общий process namespace",
    summary: "Allows containers in pod to see and signal each other's processes.",
    summaryRu: "Позволяет контейнерам в pod видеть и сигналить процессы друг друга.",
    type: "bool | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/tasks/configure-pod-container/share-process-namespace/",
    example: "shareProcessNamespace: false\n",
  },
  readinessGates: {
    title: "Readiness Gates",
    titleRu: "Readiness gates",
    summary: "Additional pod readiness conditions that must be true before pod is Ready.",
    summaryRu: "Дополнительные условия готовности pod, которые должны быть true до статуса Ready.",
    type: "YAML block string | list | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle/#pod-readiness-gate",
    example: "readinessGates: |-\n  - conditionType: example.com/ready\n",
  },
  overhead: {
    title: "Pod Overhead",
    titleRu: "Pod overhead",
    summary: "Additional resource overhead accounted for pod runtime sandbox.",
    summaryRu: "Дополнительный resource overhead, учитываемый для runtime sandbox pod-а.",
    type: "YAML block string | map | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/scheduling-eviction/pod-overhead/",
    example: "overhead: |-\n  cpu: 10m\n  memory: 32Mi\n",
  },
  clusterIPs: {
    title: "Service ClusterIPs",
    titleRu: "Список ClusterIP у Service",
    summary: "Dual-stack cluster IP list for Service (advanced networking setup).",
    summaryRu: "Список cluster IP для dual-stack Service (продвинутый сетевой сценарий).",
    type: "YAML block string | list | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/services-networking/dual-stack/",
    example: "clusterIPs: |-\n  - 10.96.0.10\n  - fd00::10\n",
  },
  externalIPs: {
    title: "Service External IPs",
    titleRu: "Внешние IP Service",
    summary: "Static external IP addresses that route traffic to Service.",
    summaryRu: "Статические внешние IP-адреса, которые маршрутизируют трафик в Service.",
    type: "YAML block string | list | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/services-networking/service/#external-ips",
    example: "externalIPs: |-\n  - 203.0.113.10\n",
  },
  ipFamilies: {
    title: "Service IP Families",
    titleRu: "IP-family Service",
    summary: "Configures preferred IP family order (`IPv4`, `IPv6`) for Service.",
    summaryRu: "Настраивает порядок IP-family (`IPv4`, `IPv6`) для Service.",
    type: "YAML block string | list | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/services-networking/dual-stack/",
    example: "ipFamilies: |-\n  - IPv4\n  - IPv6\n",
  },
  ipFamilyPolicy: {
    ...
    titleRu: "Политика IP-family Service",
    summary: "Controls single/dual-stack behavior (`SingleStack`, `PreferDualStack`, `RequireDualStack`).",
    summaryRu: "Определяет single/dual-stack поведение (`SingleStack`, `PreferDualStack`, `RequireDualStack`).",
    type: "string | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/services-networking/dual-stack/",
    example: "ipFamilyPolicy: PreferDualStack\n",
  },
  externalName: {
    title: "ExternalName Target",
    titleRu: "Целевой ExternalName",
    summary: "DNS name target for `type: ExternalName` Service.",
    summaryRu: "Целевое DNS-имя для Service типа `ExternalName`.",
    type: "string | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/services-networking/service/#externalname",
    example: "externalName: api.external.example.com\n",
  },
  loadBalancerClass: {
    title: "LoadBalancer Class",
    titleRu: "Класс LoadBalancer",
    summary: "Selects implementation class for LoadBalancer provisioning.",
    summaryRu: "Выбирает класс реализации для provision LoadBalancer.",
    type: "string | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/services-networking/service/#specifying-class-of-load-balancer-implementation",
    example: "loadBalancerClass: internal-vip\n",
  },
  loadBalancerSourceRanges: {
    title: "LoadBalancer Source Ranges",
    titleRu: "Source ranges для LoadBalancer",
    summary: "Restricts allowed client CIDRs for LoadBalancer traffic.",
    summaryRu: "Ограничивает разрешенные клиентские CIDR для LoadBalancer-трафика.",
    type: "YAML block string | list | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/services-networking/service/",
    example: "loadBalancerSourceRanges: |-\n  - 10.0.0.0/8\n  - 192.168.0.0/16\n",
  },
  healthCheckNodePort: {
    title: "Health Check NodePort",
    titleRu: "NodePort для health check",
    summary: "Optional NodePort used for external load balancer health checks.",
    summaryRu: "Опциональный NodePort для health-check внешнего load balancer.",
    type: "number | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/services-networking/service/",
    example: "healthCheckNodePort: 32080\n",
  },
  sessionAffinityConfig: {
    title: "Session Affinity Config",
    titleRu: "Конфигурация session affinity",
    summary: "Advanced settings for Service session affinity behavior.",
    summaryRu: "Продвинутые настройки поведения session affinity у Service.",
    type: "YAML block string | map | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.30/#servicespec-v1-core",
    example: "sessionAffinityConfig: |-\n  clientIP:\n    timeoutSeconds: 10800\n",
  },
  publishNotReadyAddresses: {
    title: "Publish NotReady Addresses",
    titleRu: "Публиковать NotReady адреса",
    summary: "Publishes endpoints even when backing pods are not Ready.",
    summaryRu: "Публикует endpoint-ы даже когда backing pod-ы еще не Ready.",
    type: "bool | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.30/#servicespec-v1-core",
    example: "publishNotReadyAddresses: true\n",
  },
  allocateLoadBalancerNodePorts: {
    title: "Allocate LoadBalancer NodePorts",
    titleRu: "Выделять NodePort для LoadBalancer",
    summary: "Controls whether NodePorts are allocated for LoadBalancer Service.",
    summaryRu: "Управляет тем, выделяются ли NodePort-ы для LoadBalancer Service.",
    type: "bool | env-map",
    k8sDocsLink: "https://kubernetes.io/docs/concepts/services-networking/service/#disabling-load-balancer-nodeport-allocation",
    example: "allocateLoadBalancerNodePorts: false\n",
  },
  prometheusSampleLimit: {
    title: "Prometheus Sample Limit",
    titleRu: "Лимит sample для Prometheus",
    summary: "Limit used in Strimzi monitoring resources to cap accepted samples.",
    summaryRu: "Лимит, используемый в monitoring-ресурсах Strimzi для ограничения принимаемых sample.",
    type: "number | env-map",
    example: "prometheusSampleLimit: 10000\n",
  },
  ...
  extraSpec: {
    ...
    summary: "Raw spec fragment merged into generated resource spec for advanced tuning.",
    summaryRu: "Raw-фрагмент spec, мержимый в сгенерированный spec ресурса для тонкой настройки.",
    type: "YAML block string | map | env-map",
    example: "extraSpec: |-\n  template:\n    spec:\n      hostNetwork: true\n",
  },
  zookeeper: {
